...
def localization_net(inputs):
    arg_scope = nets_factory.arg_scopes_map['cifarnet']()
    with slim.arg_scope(arg_scope):
            logits, end_points = nets_factory.networks_map['cifarnet'](
                images=inputs,
                is_training=True
            )
    return logits
...
def train(tfrecords, logdir, cfg, pretrained_model_path=None, trainable_scopes=None, checkpoint_exclude_scopes=None, restore_variables_with_moving_averages=False, restore_moving_averages=False):
    ...
    with graph.as_default():
        ...
        # inputs, labels = batch_queue.dequeue()#embeddings

        # Create localization network
        # %% Let the output size of the transformer be half of the image size
        with tf.variable_scope('locnet'):
            out_size = (cfg.IMAGE_PROCESSING.INPUT_SIZE, cfg.IMAGE_PROCESSING.INPUT_SIZE)
            output = localization_net(batch_dict['inputs'])

            # add transform layers
            w_fc_loc = tf.Variable(tf.zeros([output.shape[1], 6]), name='w_fc_loc')
            initial = np.array([[1.0, 0, 0], [0, 1.0, 0]])
            initial = initial.astype('float32')
            initial = initial.flatten()
            b_fc_loc = tf.Variable(initial_value = initial, name='b_fc_loc')

            # define layers
            h_fc_loc = (tf.matmul(output, w_fc_loc) + b_fc_loc)
            h_trans = transformer(batch_dict['inputs'], h_fc_loc, out_size)
            h_trans = tf.reshape(h_trans, [-1, out_size[0], out_size[1], 3])

            # add h_trans into tf.summary.image
            tf.summary.histogram(name='theta', values=h_fc_loc)
            tf.summary.image(name='1.before_spatial_transform', tensor=batch_dict['inputs'])
            tf.summary.image(name='2.after_spatial_transform', tensor=h_trans)
