...
def configure_inference_and_loss(inputs, labels, net, arg_scope, num_classes, is_training):
    ...
    with slim.arg_scope(arg_scope):
        with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
            with tf.variable_scope('Compact_Pooling/Logits'):
                ...
                net = slim.dropout(net, scope='Dropout_1b', is_training=is_training)
                logits = slim.conv2d(net, num_classes, [1, 1], activation_fn=None,
                                     normalizer_fn=None, scope='Conv2d_1c_1x1')
                logits = tf.squeeze(logits, [1, 2], name='SpatialSqueeze')
                predictions = slim.softmax(logits, scope='Predictions')
    
    loss = tf.losses.softmax_cross_entropy(logits=logits, onehot_labels=labels, label_smoothing=0., weights=1.0)
...
def test(tfrecords, checkpoint_path, save_dir, max_iterations, eval_interval_secs, cfg):
    ...
    with graph.as_default():
        ...
        predictions, loss = configure_inference_and_loss(inputs=batch_dict['inputs'],
